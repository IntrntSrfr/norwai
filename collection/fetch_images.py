...
for i, r in enumerate(tqdm(data[:100])):
  if 'angle' not in r or r['county_name'] == 'oslo':
    continue
  l = [(r['angle'] - 90)%360, (r['angle'] + 90)%360]
  for j, h in enumerate(l):
    url = "https://maps.googleapis.com/maps/api/streetview?size=615x640&location={},{}&fov=90&heading={}&key={}".format(r['lat'], r['lng'], h, MAPS_API_KEY)
    r_c = r.copy()
    try:
